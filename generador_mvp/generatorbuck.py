...
def kml_df_bucket(bucket_name, folder_path):
    ...
    for blob in blobs:
        if blob.name.endswith(".kml"):
            kml_data = blob.download_as_text(encoding='utf-8') #ANALIZA TODOS LOS KML DENTRO DE LA CARPETA ESPECIFICADA, EN ESTE CASO EL BUCKET.
...
def main():
    ...
    folder_path = "tu-carpeta/data"
    df = kml_df_bucket(bucket_name, folder_path)
